...
field_pattern = re.compile(
    r'^\s*(?:public\s+|private\s+|protected\s+|static\s+|final\s+|transient\s+|volatile\s+)*'
        ...
        if file.endswith('.java'):
            ...
            if class_match:
                ...
                for line in lines:
                    ...
                    if 'class ' + class_name in stripped or 'interface ' + class_name in stripped or 'enum ' + class_name in stripped:
                        class_declared = True
                    ...
                    if class_declared and brace_level == 1:
                        ...
                        if fm:
                            f_type = fm.group(1)
                            f_name = fm.group(2)
                            if f_type not in ["return", "new", "class", "interface", "enum"] and f_name not in ["if", "else", "for", "while", "return"]:
                                fields.append({"type": f_type.replace("<", "&lt;").replace(">", "&gt;"), "name": f_name})
                            continue
                        ...
                        if mm and "=" not in stripped.split("(")[0]:
                            ...
                            if ret_type is not None:
                                if ret_type not in ["return", "new", "else", "if"] and not m_name in ["if", "for", "while", "else", "switch", "catch"]:
                                    res_args = ' '.join(args.split()).replace("<", "&lt;").replace(">", "&gt;")
                                    res_ret = ret_type.strip().replace("<", "&lt;").replace(">", "&gt;") if ret_type else m_name
                                    # prevent duplicates
