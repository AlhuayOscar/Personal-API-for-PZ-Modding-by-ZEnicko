...
public class ISWorldObjectContextMenuLogic {
    ...
    public static boolean createMenuEntries(KahluaTable fetch, KahluaTable context, double player, KahluaTable worldobjects, int x, int y, boolean test) {
        ...
        if (safehouseAllowInteract) {
            ...
            boolean thumpableWindowResult = LuaHelpers.castBoolean(
    ...
    private static boolean doBedMenu(double player, boolean test, ISContextMenuWrapper contextWrapper, IsoObject bed, IsoPlayer playerObj) {
        if (bed == null) {
            ...
            if (!isSomethingTo(bed, playerObj) && !playerObj.isSitOnFurnitureObject(bed)) {
                if (test) {
                    return true;
                }
        ...
        if (door != null) {
            ...
            if (doorCurtain != null) {
                ...
                doorContextWrapper.addGetUpOption(
                    text, worldobjects, LuaManager.getFunctionObject("ISWorldObjectContextMenu.onOpenCloseCurtain"), door, playerObj.getPlayerNum()
                );
            ...
            if (door.couldBeOpen(playerObj)) {
                ...
                KahluaTable opendooroption = doorContextWrapper.addGetUpOption(
                    text, worldobjects, LuaManager.getFunctionObject("ISWorldObjectContextMenu.onOpenCloseDoor"), door, playerObj.getPlayerNum()
                );
            ...
            if (!door.IsOpen() && doorKeyId != null) {
                int doorKeyIdInt = (Integer)doorKeyId;
                if (playerInv.haveThisKeyId(doorKeyIdInt) != null
                    || !playerObj.getCurrentSquare().has(IsoFlagType.exterior) && !((IsoObject)door).getProperties().has(IsoPropertyType.FORCE_LOCKED)) {
                    if (test) {
                        return null;
                    }
            ...
            Texture var26 = Texture.trygetTexture(doorIsoObj.getSpriteName());
            if (var26 instanceof Texture) {
                doorOption.rawset("iconTexture", var26.splitIcon());
            }
    ...
    private static ISContextMenuWrapper doWindowMenu(
        ...
        if (window != null && !invincibleWindow) {
            ...
            Texture brokenGlass = Texture.trygetTexture(window.getSpriteName());
            if (brokenGlass instanceof Texture) {
                windowOption.rawset("iconTexture", brokenGlass.splitIcon());
            } else {
        ...
        if (curtain != null && !invincibleWindow) {
            ...
            if (!curtain.getSquare().getProperties().has(IsoFlagType.exterior)) {
                if (!playerObj.getCurrentSquare().has(IsoFlagType.exterior)) {
                    KahluaTable option = windowContextWrapper.addGetUpOption(
                        text, worldObjects, LuaManager.getFunctionObject("ISWorldObjectContextMenu.onOpenCloseCurtain"), curtain, playerObj.getPlayerNum()
                    );
                    ...
                    LuaHelpers.callLuaClass(
                        "ISWorldObjectContextMenu", "addRemoveCurtainOption", null, windowContext, worldObjects, curtain, playerObj.getPlayerNum()
                    );
                }
            } else {
                windowContextWrapper.addGetUpOption(
        ...
        IsoBrokenGlass brokenGlassx = (IsoBrokenGlass)fetch.rawget("brokenGlass");
        if (brokenGlassx != null) {
            windowContextWrapper.addGetUpOption(
        ...
        return item != null
            && (
                item.getType().equals("Generator")
                    ...
                    || item.getType().equals("CorpseAnimal")
            );
    }
