        ...
        if (safeHouse != null) {
            ...
            if (hitPoints == ServerOptions.instance.warSafehouseHitPoints.getValue()) {
                ...
                for (UdpConnection c : GameServer.udpEngine.connections) {
                    if (c.isFullyConnected() && (safeHouse.playerAllowed(c.getUserName()) || c.getRole().hasCapability(Capability.CanSetupSafehouses))) {
                ...
                for (UdpConnection cx : GameServer.udpEngine.connections) {
                    if (cx.isFullyConnected() && (safeHouse.playerAllowed(cx.getUserName()) || cx.getRole().hasCapability(Capability.CanSetupSafehouses))) {
