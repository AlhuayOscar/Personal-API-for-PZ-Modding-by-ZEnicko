...
public class NetworkZombieAI extends NetworkCharacterAI {
    ...
    public void set(ZombiePacket packet) {
        ...
        if (ModelManager.instance.isCreated()) {
            if (packet.target != -1) {
                packet.dirAngle = this.zombie.getAnimationPlayer().getTargetAngle();
            } else {
                packet.dirAngle = this.zombie.getAnimAngleRadians();
            }
        }
    ...
    public void preupdate() {
        if (GameClient.client) {
            ...
            byte direction = (byte)IsoDirections.fromAngle(this.zombie.getForwardDirection()).ordinal();
