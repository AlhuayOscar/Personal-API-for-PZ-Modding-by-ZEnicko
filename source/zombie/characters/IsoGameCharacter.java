...
public abstract class IsoGameCharacter
    ...
    private void registerVariableCallbacks() {
        ...
        this.setVariable(
            ...
            AimingMode.class,
            this::getAimingMode,
            owner -> "Get the character's aiming mode. REGULAR means it's free-aiming, TARGET_FOUND means it's spotted a target."
        );
                ...
                if (Rand.NextBool((int)var7) && (int)this.getBodyPartClothingDefense(part, false, false) < 100) {
                    ...
                    if (this instanceof IsoPlayer player) {
                        if (GameServer.server) {
                            ...
                            INetworkPacket.sendToRelative(PacketTypes.PacketType.PlaySound, this.getXi(), this.getYi(), "PainFromScratch", (byte)2, this);
                        } else {
                        ...
                        if (coveredParts != null) {
                            for (int j = 0; j < coveredParts.size(); j++) {
                                if (coveredParts.get(j).index() == partIndex) {
                                    if (GameServer.server) {
                                        INetworkPacket.sendToRelative(PacketTypes.PacketType.PlaySound, this.getXi(), this.getYi(), sound, (byte)0, this);
                                        return 0L;
                                    }
    ...
    public void actionStateChanged(ActionContext sender) {
        for (int ii = 0; ii < IsoGameCharacter.L_actionStateChanged.stateNames.size(); ii++) {
            DebugType.AnimationDetailed.debugln("************* stateNames: %s", IsoGameCharacter.L_actionStateChanged.stateNames.get(ii));
        }
    ...
    public float onHitByVehicleApplyDamage(BaseVehicle vehicle, float impactSpeed, boolean pushedBack) {
        if (!this.isAlive()) {
            ...
            float damageFromImpact = this.calculateDamageFromVehicleImpact(impactSpeed);
            float damage = damageFromImpact;
    ...
    protected float calculateDamageFromVehicleImpact(float impactSpeed) {
        if (this.isProne()) {
            ...
            return impactDamageAlpha < 0.0F ? 0.0F : 1.5F * PZMath.lerpFunc_EaseOutQuad(impactDamageAlpha);
        }
    }
    ...
    public void addBasicPatch(BloodBodyPartType part) {
        if (this instanceof IHumanVisual) {
            if (!(this instanceof IsoPlayer player && (GameServer.server || GameClient.client && !player.isLocalPlayer()))) {
                if (part == null) {
    ...
    public boolean addHole(BloodBodyPartType part, boolean allLayers) {
        if (!(this instanceof IHumanVisual)) {
            return false;
        } else if (!(this instanceof IsoPlayer player && (GameServer.server || GameClient.client && !player.isLocalPlayer()))) {
            if (part == null) {
                        ...
                        if (bodyPart.scratched()) {
                            ...
                            if (this instanceof IsoPlayer player) {
                                if (GameServer.server) {
                                    ...
                                    INetworkPacket.sendToRelative(
                                        PacketTypes.PacketType.PlaySound, this.getXi(), this.getYi(), "PainFromLacerate", (byte)2, this
                                    );
                                } else {
                            ...
                            if (this instanceof IsoPlayer playerx) {
                                if (GameServer.server) {
                                    ...
                                    INetworkPacket.sendToRelative(
                                        PacketTypes.PacketType.PlaySound, this.getXi(), this.getYi(), "PainFromScratch", (byte)2, this
                                    );
                                } else {
    ...
    private void renderDebugData() {
        ...
        if (this.getNetworkCharacterAI().getBooleanDebugOptions().enable.getValue()
            ...
            if (this.advancedAnimator.getRootLayer() != null) {
                float var64;
                stringDrawer.draw(font, sx, sy + (var64 = dy + 14.0F), "State: " + this.advancedAnimator.getCurrentStateName(), cxxx.r, cxxx.g, cxxx.b, cxxx.a);
                AnimationPlayer animationPlayer = this.getAnimationPlayer();
                if (animationPlayer != null) {
