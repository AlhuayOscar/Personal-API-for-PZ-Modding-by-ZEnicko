...
public enum DebugType {
    ...
    public void routedWrite(int backTraceOffset, LogSeverity logSeverity, String logText) {
        switch (logSeverity) {
            case Trace:
                this.getLogStream().trace(backTraceOffset + 1, logText);
                break;
            case Noise:
                this.getLogStream().noise(backTraceOffset + 1, logText);
                break;
            case Debug:
                this.getLogStream().debugln(backTraceOffset + 1, logText);
                break;
            case General:
                this.getLogStream().println(logText);
                break;
            case Warning:
                this.getLogStream().warn(backTraceOffset + 1, logText);
                break;
            case Error:
                this.getLogStream().error(backTraceOffset + 1, logText);
            case Off:
        }
    }
