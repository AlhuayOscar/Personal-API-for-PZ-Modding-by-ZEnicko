...
public final class DrainableComboItem extends InventoryItem implements Drainable, IUpdater {
    ...
    public void update() {
        ...
        if (!GameServer.server && this.getWorldItem() != null && RainManager.isRaining() && this.is(ItemKey.Drainable.BATH_TOWEL, ItemKey.Drainable.DISH_CLOTH)
            )
         {
            ...
            if (sq.isOutside()) {
                ...
                if (useDelta > 0.002F) {
                    ...
                    InventoryItem itemx = InventoryItemFactory.CreateItem(this.getReplaceOnDeplete());
                    sq.AddWorldInventoryItem(itemx, worldItem.getOffX(), worldItem.getOffY(), worldItem.getOffZ());
                    sq.transmitRemoveItemFromSquare(worldItem);
                    itemx.setWorldZRotation(this.getWorldZRotation());
                }
            }
        }
    }
